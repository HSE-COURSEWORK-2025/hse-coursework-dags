...
@dag(
    dag_id="preprocess_data_k8s",
    default_args=default_args,
    schedule_interval="0 * * * *",
    catchup=False,
    max_active_runs=1,
    concurrency=1,
    tags=["preprocess_data", "kubernetes"],
)
def preprocess_user_data_dag():
    ...
    KubernetesPodOperator.partial(
        task_id="run_data_preprocess",
        namespace="hse-coursework-health",
        image="preprocess_data:latest",
        cmds=["python3", "run.py"],
        get_logs=True,
...
dag_instance = preprocess_user_data_dag()
